...
For gap-filling, I see `r nrow(site_rn_table)` site-plot-research_name combinations to process in `r L2_QAQC`, representing `r length(files_to_process)` files.
...
for(i in seq_len(nrow(derived_vars))) {
    dv <- derived_vars$new_research_name[i]
    ...
    for(grp in files_needed_split) {
        message("\tLoading data for ", grp$site[1], " ", grp$plot[1], " ", grp$year[1])
        ...
        # Call CALC_DERIVED_X
        do.call(paste0("CALC_DERIVED_", dv), list(dat_needed))
        # mark as derived
        # mark research_name
        # write data
    }
