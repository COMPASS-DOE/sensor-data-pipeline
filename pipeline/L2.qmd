...
params:
  ...
  DATA_ROOT: "data_TEST/"
  # L2_VERSION example: "2-1". Do not include "v"
  L2_VERSION: "???"
...
# Construct our input and output locations
...
# Don't write plots for test data; partial workaround for
...
for(i in seq_len(nrow(derived_vars))) {
    ...
    for(grp in files_needed_split) {
        ...
        # We now have all the research-name data needed to compute derived
        # variable "dv" for this site and plot and year. BUT it's possible 
        # that the relevant instruments didn't all measure the same thing.
        # (This occurs at DLG, where we have TEROS12s measuring
...
# The site-research name table holds the unique combinations site, plot,
# and research name. We process at once all files that match each row
...
for(i in seq_len(nrow(site_rn_table))) {
    ...
    for(j in seq_len(nrow(these_files))) {
        ...
        # Ensure that the timestamp column is in correct time zone
        dat_gf$TIMESTAMP <- force_tz(dat_gf$TIMESTAMP, tzone = params$L2_DATA_TIMEZONE)
...
```{r metadata}
# We first write the overall README, i.e. for the entire L2 release,
# and then proceed folder by folder in generating the metadata files
# specific to each site-year folder.
...
```{r docs-and-code}
# Copy example code into release folder
