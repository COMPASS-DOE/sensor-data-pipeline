...
params:
  ...
  DATA_ROOT: "data_TEST/"
  # L1_VERSION example: "2-1". Do not include "v"
  L1_VERSION: "???"
  # L1_RELEASE_DATE is YYYY-MM-DD
  L1_RELEASE_DATE: "???"
  run_parallel: false
...
# Get the column metadata file. This provides descriptions
# about each column, specifies the column order of output data,
# and serves as a confirmation check for the presence of columns.
...
# L1 processes folders, not files; assemble the list of those
...
# Don't write plots for test data; this is a partial workaround for
...
Output directory is `r L1` (WRITE_PLOTS is `r WRITE_PLOTS`).
...
f <- function(dir_name, dirs_to_process, out_dir) {
    ...
    dat_raw <- read_csv_group(d, quiet = TRUE, col_types = "ccTcccccccccccccii")
...
```{r metadata}
# We first write the overall README, i.e. for the entire L1 release,
...
# Get the L1 template file used for generating specific metadata
# file for each site-year folder
...
for(dd in data_dirs) {
    ...
    # Write the final metadata file into the site_year folder 
