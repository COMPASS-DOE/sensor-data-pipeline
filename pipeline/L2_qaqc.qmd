...
params:
  ...
  DATA_ROOT: "data_TEST/"
  # ----- Parameters that don't normally change
...
This script is an intermediate step between L1 and L2. It

-   Reads in the L1 data files one by one

-   Drops NA data rows (although there shouldn't be any)
...
# Construct our list of L1 files to process and output location
...
# The QAQC table holds settings related to outlier detection 
# and MAC (mean annual cycle) for each variable
...
f <- function(fn, out_dir) {
    ...
    dat$F_drop <- rep(0L, nrow(dat)) # new flag: drop row or not
    ...
    # Complete the data: fill in (with NA) all timestamps and all
    # combinations of Instrument, Sensor, etc. This guarantees a 
    # smooth time series for each sensor
    rows <- nrow(dat_summarised)
    dat_complete <- L2_complete(dat_summarised)
    message("\tCompleted data; started with ", rows, 
            " rows...now ", nrow(dat_complete))
    
    # Interpolate across small gaps for each sensor
    # 'maxgap' controls zoo::na_approx's gap-filling behavior; 
    # we compute it based on the data interval and the max_gap_fill 
    # entry in the QAQC table
