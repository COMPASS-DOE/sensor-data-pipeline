...
f <- function(fn, out_dir) {
    ...
    dat_gapfilled <- L2_linear_gapfill(dat_complete, maxgap = maxgap)
    new_na_vals <- is.na(dat_gapfilled$Value)
    message("\t\tStarted with ", sum(na_vals), 
            " missing values...now ", sum(new_na_vals))
    smry$Filled <- sum(na_vals & !new_na_vals)
    
    dat_gapfilled$Type <- "DLR" # mark as datalogger data

    message("\tWriting...")
    write_to_folders(dat_gapfilled,
                     out_dir,
                     "L2_qaqc",
                     site = dat_gapfilled$Site[1],
                     plot = dat_gapfilled$Plot[1],
                     version = params$L2_VERSION,
                     # note: currently no L2_qaqc plots are generated
                     # the most useful ones would be showing outliers, but those data
                     # are gone by the time we get to this point. Hmm
