...
This script is an intermediate step between L0 and L1. It
...
L1_DATA_TIMEZONE <- params$L1_DATA_TIMEZONE # for convenience
...
# Construct our list of L0 files to process and output location
...
for(i in seq_along(files_to_process)) {
    ...
    if(!all(present)) {
        stop(basename(fn), ": some data file entries are missing in the design table!\n",
    ...
    if(site %in% tzt$Site) {
        ...
        if(site_tz != L1_DATA_TIMEZONE) {
            message("\tChanging timestamps from ", site_tz, " to ", L1_DATA_TIMEZONE)
            ...
            timestamps <- with_tz(timestamps, tzone = L1_DATA_TIMEZONE)
