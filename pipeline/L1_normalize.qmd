...
params:
  ...
  # We don't use "America/New_York" for L1_DATA_TIMEZONE because we want
  # all outputs to be in STANDARD time
...
tzt <- read_csv(METADATA_TZ_TABLE, col_types = "cc")
...
f <- function(fn, out_dir, design_table) {
    ...
    site <- dat$Site[1]
    if(site %in% tzt$Site) {
        site_tz <- tzt$Time_zone[which(site == tzt$Site)]
        ...
        stop("This site ", site, " is not in time zone table ", 
